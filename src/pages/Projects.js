        ...
        <NonSidebarLayout>
            <div className={`container pt-10 xl:px-32`}>
                ...
                <div className={`fixed w-full bottom-0 lef-0 right-0 pb-5 flex flex-col items-center backdrop-blur-lg bg-black bg-opacity-30`}>
                    ...
                    <span className="lg:w-9/12 md:w-10/12 w-full text-gray1 text-center text-xs md:text-sm mx-5 md:mx-0">
                        DataCanvas is you on-the-go IoT project data warehousing, visualization and analysis solution. Get started by creating a new project or migrating your existing project here.
                    </span>
                </div>
            </div>
            ...
            <PopupContainer isOpen={isAddProjectOpen} onClose={() => { }} closeFunction={toggleAddProjectModal} Icon={FaPlusCircle} title='Add New Project' closeIconVisible={true}>
                ...
                <div className="flex flex-col items-center justify-center mt-4">
                    <PillButton text="Create Project" onClick={handleProjectAdding} isPopup={true} icon={FaPlusCircle} />
                </div>
            </PopupContainer>

            {/* popup Container for project adding done - project ID showing */}
            <PopupContainer isOpen={isProjectAddingDoneOpen} onClose={() => { }} closeFunction={toggleProjectAddingDoneModal} Icon={FaCheckCircle} title='Project Addedd Successfully' closeIconVisible={true}>
                <div className="flex flex-col justify-center items-center mt-3">
                    <label className="text-gray1 text-sm">Project ID</label>
                    <input type="text" value={newProjectID} className="w-full bg-black3 border border-gray2 border-opacity-30 rounded-full text-center px-4 py-1 mt-2 text-gray2" readOnly />
                    <span className="text-gray2 text-xs text-center mt-2">This project ID should be included in data send requests</span>
                </div>

                <div className="flex flex-col items-center justify-center mt-4">
                    <PillButton text="Done" onClick={toggleProjectAddingDoneModal} isPopup={true} icon={FaCheck} />
                </div>
            </PopupContainer>
